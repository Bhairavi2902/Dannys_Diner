...
ROW_NUMBER() OVER (PARTITION BY sales.customer_id ORDER BY sales.order_date ASC) AS RW
FROM sales 
JOIN menu
ON sales.product_id=menu.product_id
)
...
WITH popular AS
(
SELECT sales.customer_id,menu.product_name,COUNT(menu.product_name) AS TOTAL,
